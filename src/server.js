      ...
      <div class="auth-container">
        ...
        <form method="POST" action="/register/delivery">
          ...
          <div class="radio-group">
            ...
            <label for="type-bike">🏍️ Bike / Motorcycle</label>
            
            <input type="radio" id="type-car" name="vehicleType" value="Car" required>
            <label for="type-car">🚗 Car / Van</label>
          </div>
...
//     switch (status) {
//         case 'Accepting Donations': return 'bg-green-100 text-green-800';
//         case 'Full Capacity': return 'bg-yellow-100 text-yellow-800';
...
// /**
//  * GET route to list all active food shelters for donation.
//  * Path: /shelters
//  */
// app.get("/shelters", async (req, res) => {
//     try {
//         // Fetch all shelters that are currently accepting donations
...
//                         Help reduce food waste! Below are the registered food shelters currently accepting donations. Click "Donate Food" to start the process.
//                     </p>
//                     <div class="space-y-6">
//                         ${shelters.length
//                             ? shelters.map(s => `
...
//                                             <span class="font-semibold">Location:</span> ${s.address}, ${s.city}
//                                         </p>
...
//                                     <p class="text-yellow-600 mt-2">Please check back later or contact admin to register a shelter.</p>
//                                 </div>
//                             `
//                         }
//                     </div>
...
// /**
//  * GET route for the donation form/process start.
//  * Path: /donate/:shelterId
//  */
...
//         return res.send(`Please <a href="/user">log in</a> to make a donation.`);
//     }
...
//              return res.send(`Sorry, ${shelter.name} is currently **${shelter.status}** and cannot accept donations.`);
//         }

//         // Using the exact HTML structure and inline styles from the user's commented block
//         res.send(`
//             <div style="font-family: sans-serif; padding: 20px; max-width: 600px; margin: auto; border: 1px solid #ccc; border-radius: 8px;">
//                 <h1 style="color: #10b981;">Donating to ${shelter.name}</h1>
//                 <p>You are about to start a donation process.</p>
//                 <p><strong>Shelter Address:</strong> ${shelter.address}, ${shelter.city}</p>
//                 <p><strong>Contact:</strong> ${shelter.contactPerson} at ${shelter.contactPhone}</p>
//                 <form action="/donate/submit" method="POST" style="margin-top: 20px;">
//                     <input type="hidden" name="shelterId" value="${shelterId}">
//                     <label for="item">Item Description (e.g., 5 lbs of leftover lasagna):</label>
//                     <input type="text" id="item" name="description" required style="width: 100%; padding: 10px; margin-bottom: 10px; border: 1px solid #ddd; border-radius: 4px;">
//                     <label for="quantity">Approximate Quantity:</label>
//                     <input type="number" id="quantity" name="quantity" min="1" required style="width: 100%; padding: 10px; margin-bottom: 20px; border: 1px solid #ddd; border-radius: 4px;">
...
//     } catch (error) {
//         console.error("Error fetching shelter for donation:", error);
//         res.status(500).send("An error occurred. Could not load donation page.");
//     }
// });

// // ====================================================================
// //                          /DONATE/SUBMIT ROUTE (PROCESSING)
// // ====================================================================

// /**
//  * POST route to handle the actual submission of the donation request.
//  * Path: /donate/submit
//  */
...
//     try {
//         // 1. Get shelterId, description, quantity from req.body (Done via destructuring)
//         // 2. Fetch User.findById(req.session.userId) to get sourceName
//         const user = await User.findById(userId);
//         if (!user) {
//             return res.status(404).send("Donor user not found. Please log in again.");
//         }

//         // 3. Create a new Donation document and save it.
//         const newDonation = new Donation({
//             shelterId,
//             sourceType: 'User',
//             sourceId: userId,
//             sourceName: user.name || user.email, // Use name or email for identification
//             items: [
//                 {
//                     description,
...
//         res.send(`
//             <h2 style="color: #10b981; font-family: sans-serif; padding: 20px;">✅ Donation Request Submitted!</h2>
//             <p style="font-family: sans-serif; padding: 0 20px;">Thank you, **${user.name || user.email}**, your donation has been submitted and is awaiting confirmation.</p>
//             <p style="margin-top: 20px; padding: 0 20px;"><a href="/shelters">← Back to Shelters List</a></p>
//         `);

//     } catch (error) {
//         console.error("Error submitting donation:", error);
//         res.status(500).send(`Failed to submit donation: ${error.message}`);
//     }
// });

// // ====================================================================
// //                          /TEST-ADD-SHELTER ROUTE (UTILITY)
// // ====================================================================

// /**
//  * DEVELOPMENT ROUTE: Manually inserts a test shelter into the database.
//  * Path: /test-add-shelter
//  */
...
//         // Hashing the password is necessary for any subsequent login logic
...
//         let existing = await FoodShelter.findOne({ name: testShelterName });

//         if (!existing) {
//             await FoodShelter.create(testShelterData);
//             return res.send(`
//                 <h2 style="color: green;">✅ Success!</h2>
//                 <p>Inserted **${testShelterData.name}** into the database.</p>
//                 <p><a href="/shelters">Click here to view shelters</a></p>
//             `);
//         } else {
//             return res.send(`
//                 <h2 style="color: blue;">ℹ️ Already Exists</h2>
//                 <p>**${testShelterData.name}** is already in the database.</p>
//                 <p><a href="/shelters">Click here to view shelters</a></p>
//             `);
//         }

//     } catch (error) {
//         console.error("Test shelter creation failed:", error);
//         res.status(500).send(`Error inserting test data: ${error.message}. Make sure the FoodShelter model and necessary fields exist.`);
//     }
// });

// // ====================================================================
// //                          /ORDERS/MY ROUTE (CUSTOMER ORDERS)
// // ====================================================================

// /**
//  * Route to fetch customer's current orders.
//  * Path: /orders/my
//  */
// app.get("/orders/my", async (req, res) => {
//     const userId = req.session.userId;

//     // This is the JSON endpoint, so we return JSON errors
...
//         const customerOrders = await LiveOrder.find({ userEmail: user.email })
//             // Assumes LiveOrder refers to a Restaurant model
//             .populate("restaurantId", "name")
//             .sort({ createdAt: -1 })
//             .lean();

//         const ordersData = customerOrders.map(o => ({
//             _id: o._id.toString().slice(-6),
//             restaurantName: o.restaurantId?.name || "Unknown Restaurant",
...
// app.get("/shelters", async (req, res) => {
//     // Fetch all shelters that are currently accepting donations
//     // You can adjust the query to fetch all if needed: await FoodShelter.find().lean();
//     const shelters = await FoodShelter.find({ status: "Accepting Donations" }).lean();
...
//         // This is where you would load a form allowing the user to specify
//         // the type and quantity of food items they wish to donate.
...
// /**
//  * POST route to handle the actual submission of the donation request.
//  * You will need to implement this fully, including fetching user details and saving to the Donation model.
//  */
// app.post("/donate/submit", async (req, res) => {
//    // Implementation required: 
//    // 1. Get shelterId, description, quantity from req.body
//    // 2. Fetch User.findById(req.session.userId) to get sourceName
//    // 3. Create a new Donation document and save it.
//    // 4. Redirect to a success page or the orders page.
// });
...
//     try {
//         const testShelterData = {
//             name: "Community Food Bank East",
//             // Since you need a password field for login, we add a dummy hashed one here:
//             password: "thisWillBeReplacedWithAHashInRealSetupButNeedsToBePresent", // Adjust as needed
//             address: "123 Hunger Relief St",
//             city: "Metropolis",
//             contactPerson: "Jane Doe",
//             contactPhone: "555-123-9876",
...
//         // Check if it already exists to prevent duplicates
//         let existing = await FoodShelter.findOne({ name: testShelterData.name });

//         if (!existing) {
//             // NOTE: If you added a password to the FoodShelter model, you MUST
//             // hash it before saving, just like in the /register/shelter route.
//             // For a quick test, we'll bypass the hash, but be aware of this.
...
app.get("/restaurants", async (req, res) => {
    ...
    const restaurantsJson = JSON.stringify(rests.map(r => ({
        // Ensure _id is converted to string for client-side JavaScript usage
        _id: r._id.toString(), 
        name: r.name,
        type: r.type || "Normal", 
    ...
    <head>
        ...
        <script src="https://cdn.tailwindcss.com"></script>
        <style>
            /* Custom styles for modal and status tags (Tailwind is integrated, not replaced) */
            ...
            .orders-overlay {
                ...
                justify-content: center;
                align-items: center;
                z-index: 50; /* Tailwind z-index for modals */
            }
            .order-item {
            ...
            .status-preparing { background-color: #fef3c7; color: #b45309; } /* yellow-100 / amber-700 */
            .status-ready { background-color: #dbeafe; color: #1e40af; }     /* blue-100 / blue-800 */
            .status-out { background-color: #cffafe; color: #065f46; }       /* cyan-100 / emerald-800 */
    ...
    <body class="bg-gray-50">
        <div class="max-w-7xl mx-auto p-6">
            ...
            <div class="mb-8">
                <input type="text" id="search-input" placeholder="Search by restaurant name, cuisine (e.g., Italian), or type (Home/Normal)..."
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" id="restaurant-list">
            </div>
        </div>
        ...
        <script>
            ...
            // Improved Restaurant Card Rendering
            ...
            // Fetches and displays all orders in the modal
            ...
            function renderRestaurants(restaurantsToRender) {
                ...
                if (restaurantsToRender.length === 0) {
                    restaurantList.innerHTML = '<div class="col-span-full p-10 text-center text-gray-500 text-lg">Sorry, no restaurants matched your search criteria.</div>';
                    return;
                }
            ...
            function filterRestaurants() {
                ...
                const filtered = allRestaurants.filter(r => {
                    const searchString = \`\${r.name} \${r.type} \${r.cuisine} \${r.address}\`.toLowerCase();
